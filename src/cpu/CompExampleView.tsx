...
export const CompExampleView: React.FC = () => {
    ...
    function onRunAllTestsClicked() {
        ...
        for (
            ;
            repeatCount < 100 && successCount === totalCount;
            repeatCount++
        ) {
            for (let test of examples) {
                    ...
                    if (exeModel.runArgs.halt) {
                        ...
                        if (resRegValue !== 44 && resRegValue !== 911) {
                            console.log(
                                `--- test '${
                                    test.name
                                }' halted with unknown result in reg[a0]: ${ensureSigned32Bit(
                                    resRegValue
                                )} ---`
                            );
                        } else {
                            let isSuccess =
                                (resRegValue === 44) !== test.expectFail;
                            ...
                            if (isSuccess) {
                                ...
                                console.log(
                                    `--- test '${test.name}' halted with FAILURE (test ${testNumValue}) ---`
                                );
                            }
                        }
                        completed = true;
                ...
                if (!completed) {
                    console.log(
                        `--- test '${test.name}' halted after too many instructions ---`
                    );
                }
            }
        }
        ...
        console.log(
            `All tests done in ${timeMs.toFixed(
                1
            )}ms. Success: ${successCount}/${totalCount} (repeats=${repeatCount}). Instructions: ${insCount} (${(
                insCount / timeMs
            ).toFixed(0)} kHz)`
        );
    ...
    function getPcComp() {
        return findCompByDefId("core/flipflop/reg1") as
            | IExeComp<ICompDataSingleReg>
            | undefined;
    }
    function getRegsComp() {
        return findCompByDefId("core/riscv/reg32") as
            | IExeComp<ICompDataRegFile>
            | undefined;
    }
    function getRomComp() {
        return findCompByDefId("core/mem/rom0") as
            | IExeComp<IRomExeData>
            | undefined;
    }
    ...
    return (
        <div className={s.exampleView}>
            <div className={s.header}>
                Examples
                <div
                    className={s.reloadBtn}
                    onClick={() => setReloadCntr((a) => a + 1)}
                >
            ...
            <div className={s.body}>
                <button
                    className={s.btn}
                    disabled={exeModel.runArgs.halt}
                    onClick={onStepClicked}
                >
                    Step
                </button>
                <button className={s.btn} onClick={onResetClicked}>
                    Reset
                </button>
                <button className={s.btn} onClick={onRunAllTestsClicked}>
                    Run all
                </button>
                <button className={s.btn} onClick={runTestsQuickly}>
                    Run all (slow)
                </button>
            </div>
        </div>
